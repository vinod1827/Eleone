...
import com.morgat.eleone.listeners.ProgressBarListener;
import com.morgat.eleone.components.SegmentedProgressBar;
...
import com.morgat.eleone.utils.MergeVideoAudio;
...
public class VideoRecordingActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void Start_or_Stop_Recording(){
        ...
        if (!is_recording && sec_passed<(Variables.recording_duration/1000)-1) {
            ...
            File file = new File(Variables.getRootPath(this) + "/" + "myvideo"+(number)+".mp4");
            videopaths.add(Variables.getRootPath(this) + "/" + "myvideo"+(number)+".mp4");
    ...
    private boolean append() {
       final ProgressDialog progressDialog=new ProgressDialog(VideoRecordingActivity.this);
            ...
            public void run() {
                ...
                for (int i=0;i<videopaths.size();i++){
                    ...
                    if(file.exists()) {
                        ...
                        retriever.setDataSource(VideoRecordingActivity.this, Uri.fromFile(file));
                ...
                try {
                    ...
                    String outputFilePath=null;
                            if(audio!=null){
                                outputFilePath=Variables.getRootPath(VideoRecordingActivity.this)+Variables.outputfile;
                            }else {
                                outputFilePath=Variables.getRootPath(VideoRecordingActivity.this)+Variables.outputfile2;
                            }
    ...
    public void Merge_withAudio(){
        ...
        String root = Variables.getRootPath(this);
        String audio_file;
            audio_file =Variables.getRootPath(this)+Variables.SelectedAudio_AAC;

        String video = root + "/"+"output.mp4";
        String finaloutput = root + "/"+"output2.mp4";

        MergeVideoAudio merge_video_audio=new MergeVideoAudio(VideoRecordingActivity.this);
    ...
    public  void PreparedAudio(){
        File file=new File(Variables.getRootPath(this)+ Variables.SelectedAudio_AAC);
        if(file.exists()) {
            audio = new MediaPlayer();
            try {
                audio.setDataSource(Variables.getRootPath(this) + Variables.SelectedAudio_AAC);
                audio.prepare();
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    public void DeleteFile(){
        delete_count++;
        File output = new File(Variables.getRootPath(this)+Variables.outputfile);
        File output2 = new File(Variables.getRootPath(this)+Variables.outputfile2);
        File output_filter_file = new File(Variables.getRootPath(this)+Variables.output_filter_file);
        ...
        File file = new File(Variables.getRootPath(this) + "/" + "myvideo"+(delete_count)+".mp4");
        if(file.exists()){
            file.delete();
            DeleteFile();
        }
